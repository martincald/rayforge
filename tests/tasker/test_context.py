...
class TestExecutionContext:
    def test_root_context_initialization(self):
        update_cb = Mock()
    ...
    def test_update_scheduling_and_throttling(self, mock_timer_factory):
        ctx = ExecutionContext(
    ...
    def test_fire_update(self, mock_timer_factory):
        update_cb = Mock()
        mock_scheduler = Mock()
        ctx = ExecutionContext(
        ...
        # message
        mock_scheduler.assert_called_once_with(update_cb, 0.5, "Update fired")
        assert ctx._pending_progress is None
        assert ctx._pending_message is None
        assert ctx._update_timer is None

    def test_fire_update_when_cancelled(self, mock_timer_factory):
        update_cb = Mock()
        mock_scheduler = Mock()
        ctx = ExecutionContext(
            update_callback=update_cb,
    ...
    def test_flush(self, mock_timer_factory):
        update_cb = Mock()
        mock_scheduler = Mock()
        ctx = ExecutionContext(
            update_callback=update_cb, scheduler=mock_scheduler
        ...
        mock_scheduler.assert_called_once_with(update_cb, 0.75, "Flushing")
        assert ctx._pending_progress is None
        assert ctx._pending_message is None

    def test_flush_when_nothing_pending(self):
        update_cb = Mock()
        mock_scheduler = Mock()
        ctx = ExecutionContext(
            update_callback=update_cb, scheduler=mock_scheduler
    ...
    def test_sub_context_initialization(self):
        ...
        assert sub._scheduler is None  # Delegates to parent
    ...
    def test_sub_context_flush(self, mock_timer_factory):
        ...
        mock_scheduler.assert_called_once_with(update_cb, 0.5, None)
        assert root._pending_progress is None
