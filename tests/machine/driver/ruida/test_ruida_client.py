...
class StubJogTransport:
    """Jog-channel stand-in that records raw and framed sends apart."""
...
class TestRuidaClientSetOrigin:
    """set_origin presses the panel Origin key on the jog channel."""
    ...
    @pytest.mark.asyncio
    async def test_set_origin_emits_key_down_then_key_up(self):
        """Exactly two raw datagrams: a5 50 08 then a5 51 08."""
        await self.client.set_origin()

        assert self.jog.raw_sent == [b"\xa5\x50\x08", b"\xa5\x51\x08"]

    @pytest.mark.asyncio
    async def test_set_origin_bypasses_swizzle_and_checksum(self):
        """Origin keys go out raw, not through send_command framing."""
        await self.client.set_origin()

        assert self.jog.commands_sent == []

    @pytest.mark.asyncio
    async def test_set_origin_sends_nothing_on_main_channel(self):
        """The main command channel stays silent."""
        await self.client.set_origin()

        assert self.main.sent == []

    @pytest.mark.asyncio
    async def test_set_origin_without_jog_transport_raises(self):
        """A client with no jog channel cannot press the panel key."""
        client = RuidaClient(StubJobTransport())  # type: ignore[arg-type]
