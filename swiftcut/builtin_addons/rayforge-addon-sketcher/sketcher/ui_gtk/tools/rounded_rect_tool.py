...
class RoundedRectTool(SketchTool):
    ...
    def handle_text_input(self, text: str) -> bool:
        """Handle numeric input for setting rounded rectangle dimensions."""
        if self._preview_state is None:
            return False
    ...
    def _apply_field_constraint(self, field_index: int) -> None:
        ...
        if field_index == 0:
            ...
            self._preview_state.set_dimensions(registry, radius=value)

    def _apply_dimension_input(self):
        """Apply the dimension input to the preview rounded rectangle."""
        if self._preview_state is None:
            self._dim_input.cancel()
            return
        ...
        radius = values[2] if len(values) > 2 else None

        if width is not None or height is not None or radius is not None:
            self._preview_state.set_dimensions(
                registry, width=width, height=height, radius=radius
            )
        self._finalize_shape(
            fixed_width=width, fixed_height=height, fixed_radius=radius
        )
        self.element.mark_dirty()
    ...
    def _finalize_shape(
        ...
        radius = (
            ...
            else self._preview_state.radius
        )
        try:
