...
from sketcher.core.constraints import HorizontalConstraint
...
def test_horizontal_targets_segment(setup_env):
    ...
    # Base implementation returns False (Horizontal is topological)
...
def test_horizontal_can_apply_to_two_points():
    sketch = Sketch()
    ...
    assert HorizontalConstraint.can_apply_to(selection, sketch) is True


def test_horizontal_can_apply_to_single_line():
    sketch = Sketch()
    p1 = sketch.add_point(0, 0)
