...
def test_grid_tool_show_dialog_creates_command_on_create(
    ...
    with (
        patch("gi.repository.Adw.MessageDialog") as MockDialog,
        patch("sketcher.ui_gtk.tools.grid_tool.SpinRow") as MockSpinRow,
        patch("gi.repository.Gtk.ListBox") as MockListBox,
    ):
        MockDialog.return_value = mock_dialog
        MockSpinRow.side_effect = [mock_rows_row, mock_cols_row]
        mock_rows_row.get_int_value.return_value = 4
        mock_cols_row.get_int_value.return_value = 5
...
def test_grid_tool_show_dialog_handles_invalid_input(grid_tool, mock_element):
    ...
    with (
        ...
        MockDialog.return_value = mock_dialog
        MockSpinRow.side_effect = [mock_rows_row, mock_cols_row]
        mock_rows_row.get_int_value.return_value = 2
        mock_cols_row.get_int_value.return_value = 2
        MockListBox.return_value = MagicMock()
...
def test_grid_tool_show_dialog_handles_too_small_grid(grid_tool, mock_element):
    ...
    with (
        ...
        MockSpinRow.side_effect = [mock_rows_row, mock_cols_row]
        mock_rows_row.get_int_value.return_value = 2
        mock_cols_row.get_int_value.return_value = 3
        MockListBox.return_value = MagicMock()
